...
def run_lumis(json, run="", trg="", normtag="BRIL"):
    ...
    brilcall = [f"brilcalc lumi {'-r'+ str(run) if run else ''} -u /fb -i {json} {'--hltpath '+ str(trg)+'_v*' if trg else ''} --output-style csv --normtag /cvmfs/cms-bril.cern.ch/cms-lumi-pog/Normtags/normtag_{normtag}.json"]
